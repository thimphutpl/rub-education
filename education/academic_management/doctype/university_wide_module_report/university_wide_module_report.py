...
class UniversityWideModuleReport(Document):
	...
	if TYPE_CHECKING:
		from erpnext.university_wide_module_report.doctype.university_wide_module_report_reviewer.university_wide_module_report_reviewer import UniversityWideModuleReportReviewer
		from erpnext.university_wide_module_report.doctype.note_copy.note_copy import NoteCopy
		from erpnext.university_wide_module_report.doctype.note_remark.note_remark import NoteRemark
		...
		university_wide_module_report_format: DF.Data | None
		university_wide_module_report_series: DF.Link
		university_wide_module_report_type: DF.Link
		...
		reviewers: DF.Table[UniversityWideModuleReportReviewer]
		title: DF.Data
	...
	def on_submit(self):
		self.university_wide_module_report_format = make_autoname(str(self.university_wide_module_report_series)+".YYYY./.#####")
		frappe.db.set_value("UniversityWideModuleReport", self.name, "university_wide_module_report_format", self.university_wide_module_report_format)
	...
	def workflow_action(self):  
		...
		if action in ("Forward","Apply"):
			...
			message = "UniversityWideModuleReport Document Successfully Forwarded to {}".format(self.forward_to)
		...
		if action in ("Approve","Reject"):
			...
			message = "UniversityWideModuleReport Document {}".format("Approved" if action == "Approve" else "Rejected")
	...
	def notify_reviewer(self):
		...
		template = frappe.db.get_single_value('HR Settings', 'university_wide_module_report_reviewer_notification')
		if not template:
			frappe.msgprint(_("Please set default template for UniversityWideModuleReport Reviewer Notification in HR Settings."))
			return
	...
	def notify_employee(self):
		...
		template = frappe.db.get_single_value('HR Settings', 'university_wide_module_report_status_notification')
		if not template:
			frappe.msgprint(_("Please set default template for UniversityWideModuleReport Status Notification in HR Settings."))
			return
	...
	def notify_copy_to(self):
		...
		if len(self.copied) > 0:
			template = frappe.db.get_single_value('HR Settings', 'university_wide_module_report_copyto_notification')
			if not template:
				frappe.msgprint(_("Please set default template for UniversityWideModuleReport Copy To in HR Settings."))
				return
			email_template = frappe.get_doc("Email Template", template)
			message = frappe.render_template(email_template.response, args)
			subject = email_template.subject   
	...
	def notify_approval(self):
		args = self.get_args()
		template = frappe.db.get_single_value('HR Settings', 'university_wide_module_report_approval_notification')
		if not template:
			frappe.msgprint(_("Please set default template for UniversityWideModuleReport Approval Notification in HR Settings."))
			return
		...
		return [
			{
				...
				"name": self.name,
				"print_format": "UniversityWideModuleReport",
	...
	def restore_content(self,child_id):
		doc = frappe.get_doc("UniversityWideModuleReport", self.name)
	...
	def update_status(self):
		doc = frappe.get_doc("UniversityWideModuleReport", self.name)
		query = "SELECT name from `tabUniversityWideModuleReport Reviewer` Where user_id == '{}'".format(frappe.session.user)
		sql = frappe.db.sql(query)

		doc = frappe.db.get_doc("UniversityWideModuleReport Reviewer", sql[0].name)
		doc.status = "Reviewed"
		doc.save()
	...
	def save_remark(self, remark, reviewers=None):
		...
		""".format(self.name, frappe.session.user), as_dict=True)
		if not db_check:
			doc1 = frappe.get_doc("UniversityWideModuleReport", self.name)
			doc1.append("remark",{
				"employee":employee,
				"employee_name": employee_name,
				"user": frappe.session.user,
		...
		if reviewers:
			...
			frappe.db.set_value("UniversityWideModuleReport Reviewer", {'user_id': frappe.session.user}, "status", "Reviewed")
			query = """SELECT * FROM `tabUniversityWideModuleReport Reviewer` WHERE parent = '{0}'""".format(self.name)
			...
			if status == 1:
				...
				# doc.save()
				# frappe.db.commit()
				frappe.db.set_value("UniversityWideModuleReport", {'name': self.name}, "workflow_state", "Pending")
...
def get_permission_query_conditions(user):
	...
	# reviewers  = frappe.db.sql("SELECT user_id FROM `tabUniversityWideModuleReport Reviewer")
    ...
    if "HR User" in user_roles or "HR Manager" in user_roles:
        return
    return """(
        `tabUniversityWideModuleReport`.owner = '{user}' or
		IF (
				(`tabUniversityWideModuleReport`.reviewer_required = '1' and `tabUniversityWideModuleReport`.workflow_state != 'Waiting for Reviewer') or `tabUniversityWideModuleReport`.reviewer_required = '0',  
				`tabUniversityWideModuleReport`.permitted_user = '{user}',
				exists(select 1
					from `tabEmployee` e, `tabUniversityWideModuleReport Reviewer` nr
					where e.user_id = '{user}' and '{user}' = nr.user_id and nr.parent = `tabUniversityWideModuleReport`.name)
			)
		or
        exists(select 1
			from `tabEmployee` e, `tabNote Copy` nc
			where e.user_id = '{user}' and '{user}' = nc.user_id and nc.parent = `tabUniversityWideModuleReport`.name)
   		)
		""".format(user=user)
# `tabUniversityWideModuleReport`.permitted_user = '{user}' or
# or
# 		exists(select 1
# 			from `tabEmployee` e, `tabUniversityWideModuleReport Reviewer` nr
# 			where e.user_id = '{user}' and '{user}' = nr.user_id and nr.parent = `tabUniversityWideModuleReport`.name)
