...
def get_program_enrollment(
	...
	if program:
		condition1 += " and s.programme = %(program)s"
	# if batch:
	# 	condition1 += " and pe.student_batch_name = %(batch)s"
	if batch:
		condition1 += " and s.student_batch = %(batch)s"
	# if student_category:
	...
	# 	from
	# 		`tabStudent` s {condition2}
	# 	where
	# 		s.status = 'Active'
	# 		and s.programme = {program}
	# 		and s.student_batch = {batch}
	# 	order by
	# 		s.student_name asc
	# 	""".format(
...
def fetch_students(doctype, txt, searchfield, start, page_len, filters):
	if filters.get("group_based_on") != "Activity":
		...
		student_section_student = frappe.db.sql_list(
			"""select student from `tabStudent Group Student` where parent=%s""",
			(filters.get("student_section")),
		)
		students = (
			[d.student for d in enrolled_students if d.student not in student_section_student]
			if enrolled_students
			else [""]
		) or [""]
		return frappe.db.sql(
