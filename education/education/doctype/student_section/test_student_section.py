...
class TestStudentSection(FrappeTestCase):
	def setUp(self):
		create_academic_year()
		create_academic_term(
	...
	def test_student_roll_no(self):
		student_section = frappe.get_doc("Student Group", "Test Student Group")
		self.assertEqual(
			max([d.group_roll_number for d in student_section.students]),
			len(student_section.students),
		)
