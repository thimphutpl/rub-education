...
function fetch_researcher_name(frm, cdt, cdn, type, researcher_id) {
    frappe.call({
        method: "frappe.client.get_value",
        args: {
            doctype: type,
            filters: {"name": researcher_id},
            fieldname: type === "Employee" ? "employee_name" : "student_name"
        },
        callback: function(response) {
            if (response.message) {
                let name_field = type === "Employee" ? "employee_name" : "student_name";
                let researcher_name = response.message[name_field];
                frappe.model.set_value(cdt, cdn, "researcher_name", researcher_name);
            } else {
                frappe.model.set_value(cdt, cdn, "researcher_id", "");
                frappe.model.set_value(cdt, cdn, "researcher_name", "");
                frappe.msgprint(__("Invalid {0}: {1}", [type, researcher_id]));
            }
        }
    });
}
